...
async function trailStopLossForLong(symbol, tradeDetails, currentPrice) {
  try {
    ...
    if (roi >= TRAILING_START_ROI) {
      const targetROI = roi - 1; // Removed redundant if-else (both did same)
      ...
      if (newStop >= roundedCurrent) {
        console.warn(
          `[${symbol}] Skipping SL update — newStop (${newStop}) >= currentPMI (${roundedCurrent})`
        );
        return;
      }
      ...
      if (newStop > oldStop) {
        ...
        // Cleanup existing STOP_MARKET orders
        let openOrders;
        try {
          ...
          for (const order of openOrders) {
            if (order.type === 'STOP_MARKET' && order.side === 'SELL' && order.reduceOnly) {
              try {
                ...
                if (cancelErr.code === -2011) {
                  console.log(`[${symbol}] Order ${order.orderId} already gone (-2011). Skipping.`);
                } else {
        ...
        try {
          ...
          if (err.code === -2011) {
            console.log(`[${symbol}] Old stop ${stopLossOrderId} already gone (-2011). Proceeding.`);
          } else {
        ...
        try {
          ...
          } else if (placeErr.code === -2011) {
            console.warn(`[${symbol}] Place failed (-2011). Skipping this update.`);
