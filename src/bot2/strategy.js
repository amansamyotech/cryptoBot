...
const { EMA, RSI, MACD, ADX,  SMA } = require("technicalindicators");
const { getCandles } = require("../helper/getCandles.js"); // Adjust path if needed
...
function detectEngulfingPattern(candles) {
  ...
  const bullishEngulfing = previous.close < previous.open && // previous red
  ...
  const bearishEngulfing = previous.close > previous.open && // previous green
...
async function checkEntrySignal(symbol) {
  try {
    ...
    const longCondition = 
      ...
      (engulfingPattern.bullishEngulfing || currentPrice > previousPrice * 1.001); // Price action confirmation
    ...
    const shortCondition =
      ...
      (engulfingPattern.bearishEngulfing || currentPrice < previousPrice * 0.999); // Price action confirmation
