...
// 📊 Calculate indicators with additional signals
async function getIndicators(symbol) {
  const candles = await getCandles(symbol, interval, 100);
  const closes = candles.map((c) => c.close);
  const highs = candles.map((c) => c.high);
...
// 🧠 Enhanced Trade Direction Decision (More Aggressive & Accurate)
async function decideTradeDirection(symbol) {
  const ind = await getIndicators(symbol);
  ...
  if (ind.bbUpper !== null && ind.bbLower !== null && ind.bbMiddle !== null) {
    ...
    const bbPosition = (currentPrice - ind.bbLower) / (ind.bbUpper - ind.bbLower);
  ...
  if (totalBullish > totalBearish && scoreAdvantage >= minAdvantage && confidence >= minConfidence) {
    console.log(`🟢 DECISION: LONG`);
    return "LONG";
  }
  
  if (totalBearish > totalBullish && scoreAdvantage >= minAdvantage && confidence >= minConfidence) {
    console.log(`🔴 DECISION: SHORT`);
    return "SHORT";
  }

  console.log(`⚪ DECISION: HOLD (Advantage: ${scoreAdvantage}, Confidence: ${confidence})`);
  return "HOLD";
}
...
async function placeBuyOrder(symbol, maxSpend) {
  ...
  const lossAmount = investedAmount * 0.01; // 1%
  const profitAmount = investedAmount * 0.02; // 2%
  ...
  for (const sym of symbols) {
    try {
      ...
      if (status == true) {
        ...
        console.log(`TRADE ALREADY OPEN FOR THAT SYMBOL : ${sym} `);
      }
    } catch (err) {
