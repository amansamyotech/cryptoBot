...
// // Method 1: Multi-point angle calculation (recommended) - Including Current Candle
// function getTEMAAngleMultiPoint(temaArray, lookbackPeriods = 3) {
//   const len = temaArray.length;
//   if (len < lookbackPeriods + 1) return 0;

//   const current = temaArray[len - 1]; // Include current forming candle
//   const previous = temaArray[len - 1 - lookbackPeriods];
...
// // Method 2: Rate of change approach - Including Current Candle
// function getTEMAAngleROC(temaArray, periods = 3) {
//   const len = temaArray.length;
...
// // Method 3: Linear regression slope - Including Current Candle
// function getTEMAAngleRegression(temaArray, periods = 5) {
//   const len = temaArray.length;
//   if (len < periods) return 0;

//   // Get last N TEMA values (including current forming candle)
...
// // Method 4: TradingView-like calculation - Including Current Candle
// function getTEMAAngleTradingView(
//   temaArray,
//   timeframeMinutes = 3,
//   lookbackPeriods = 2
...
// // Updated decision function - Using Current Forming Candle
// async function decideTradeDirection300(symbol) {
//   try {
//     const pastCandles5m = await getCandles(symbol, TIMEFRAME_MAIN, 1000);
//     if (pastCandles5m.length < 50) {
...
//     if (isSidewaysMarket(pastCandles5m)) {
//       console.log(`Market is sideways for ${symbol}. Decision: HOLD`);
//       return "HOLD";
//     }

//     const pastCandles3m = await getCandles(symbol, "3m", 35); // Get more candles
//     if (pastCandles3m.length < 30) {
//       console.log("Not enough 3m candles for TEMA angle check");
//       return "HOLD";
//     }
...
//     // Try different angle calculation methods (all using current forming candle)
//     const angleMultiPoint = getTEMAAngleMultiPoint(tema15, 4);
//     const angleROC = getTEMAAngleROC(tema15, 3);
//     const angleRegression = getTEMAAngleRegression(tema15, 5);
//     const angleTradingView = getTEMAAngleTradingView(tema15, 3, 2);

//     console.log(`TEMA Angles for ${symbol} (Including Current Forming Candle):`);
//     console.log(`   Multi-point (4 periods): ${angleMultiPoint.toFixed(2)}°`);
//     console.log(`   Rate of Change: ${angleROC.toFixed(2)}°`);
//     console.log(`   Linear Regression: ${angleRegression.toFixed(2)}°`);
//     console.log(`   TradingView-like: ${angleTradingView.toFixed(2)}°`);
...
//     console.log(
//       `No valid signal. Angle: ${selectedAngle.toFixed(2)}° (Including Current)`
//     );
//     return "HOLD";
//   } catch (err) {
//     console.error(`Decision error for ${symbol}:`, err.message);
...
//   console.log(`Starting real-time monitoring for ${symbol} every ${intervalMs/1000} seconds...`);
//   setInterval(async () => {
//     await decideTradeDirection300(symbol); // This will fetch fresh data and log current angles
//   }, intervalMs);
// }

// module.exports = { decideTradeDirection300, monitorTEMAAngle };
