...
function checkVolumeSpike(candles, lookback = 5) {
  if (candles.length < lookback + 1) return false;
  const avgVol = candles.slice(-lookback - 1, -1).reduce((sum, c) => sum + c.volume, 0) / lookback;
  const lastVol = candles[candles.length - 1].volume;
  return lastVol > avgVol * 1.15;
}
...
async function decideTradeDirection(symbol, candles1m, candles5m, candleIndex) {
  try {
    ...
    if (Math.abs(ema9Angle) < MIN_ANGLE_THRESHOLD && Math.abs(ema15Angle) < MIN_ANGLE_THRESHOLD) {
      return "HOLD";
    }
    const longConditions = [
      ema9 > ema15,
...
async function backtest(symbol, startDate, endTime) {
  const startTime = new Date(startDate).getTime();
  ...
  for (let i = 50; i < candles1m.length - 1; i++) {
    ...
    if ((signal === "LONG" || signal === "SHORT") && !position) {
      ...
      console.log(`Ignored ${signal} signal at ${new Date(currentCandle.openTime).toISOString()} (position already open)`);
    }
    if (position) {
      const nextCandle = candles1m[i + 1];
      const currentPrice = nextCandle.close;
