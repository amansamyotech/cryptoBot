...
async function updateDecideLogicSir(symbol) {
  try {
    // Get candles for 3m, 15m, 1h
    const candles3m = await getCandles(symbol, "3m", 500);
    const candles15m = await getCandles(symbol, "15m", 500);
    const candles1h = await getCandles(symbol, "1h", 500);
    ...
    // Check sideways on main TF (3m)
    if (isSidewaysMarket(candles3m)) {
      console.log(`⚖️ Market sideways on 3m for ${symbol}. HOLD`);
      return "HOLD";
    }

    // Function to calculate TEMA-based signal for a given timeframe
    const checkTEMA = (candles, tf) => {
      const closes = candles.map((c) => c.close);
      const tema25 = calculateTEMA(closes, 25);
      if (tema25.length < 2) return "HOLD";

      const lastPrice = closes.at(-1);
      const lastTEMA25 = tema25.at(-1);
      const prevTEMA25 = tema25.at(-2);

      const slope = ((lastTEMA25 - prevTEMA25) / prevTEMA25) * 1000;
      const angle = Math.atan(slope) * (180 / Math.PI);

      if (lastPrice > lastTEMA25 && angle > 30) return "LONG";
      if (lastPrice < lastTEMA25 && angle < -30) return "SHORT";
      return "HOLD";
    };

    // Signals from different timeframes
    const sig3m = checkTEMA(candles3m, "3m");
    const sig15m = checkTEMA(candles15m, "15m");
    const sig1h = checkTEMA(candles1h, "1h");
...
module.exports = { updateDecideLogicSir, calculateTEMA };
