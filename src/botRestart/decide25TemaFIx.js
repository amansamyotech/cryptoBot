...
async function decide25TemaFIx(symbol) {
  try {
    ...
    if (tema25.length < 2) {
      console.log(`❌ Insufficient TEMA data for ${symbol}`);
      return "HOLD";
    }
    ...
    // Decision logic: Check if candle is above/below TEMA(25) and angle matches
    let decision = "HOLD";
    let reason = "";

    // Check if the entire candle (open, high, low, close) is above TEMA(25) for LONG
    const isCandleAboveTEMA = currentCandle.low > lastTEMA25;
    // Check if the entire candle (open, high, low, close) is below TEMA(25) for SHORT
    const isCandleBelowTEMA = currentCandle.high < lastTEMA25;

    if (isCandleAboveTEMA && angleDegrees > 35) {
      decision = "LONG";
      reason = `Candle (Low: ${currentCandle.low.toFixed(
        2
      )}) is above TEMA(25) (${lastTEMA25.toFixed(
        2
      )}) and angle ${angleDegrees.toFixed(2)}° > 35°`;
    } else if (isCandleBelowTEMA && angleDegrees < -35) {
      decision = "SHORT";
      reason = `Candle (High: ${currentCandle.high.toFixed(
        2
      )}) is below TEMA(25) (${lastTEMA25.toFixed(
        2
      )}) and angle ${angleDegrees.toFixed(2)}° < -35°`;
    } else {
      reason = `Conditions not met. Candle: [Open: ${currentCandle.open.toFixed(
        2
      )}, High: ${currentCandle.high.toFixed(
        2
      )}, Low: ${currentCandle.low.toFixed(
        2
      )}, Close: ${currentCandle.close.toFixed(
        2
      )}], TEMA(25): ${lastTEMA25.toFixed(2)}, Angle: ${angleDegrees.toFixed(
        2
      )}°`;
    }
    ...
    console.error(`❌ Error in decide25TemaFIx for ${symbol}:`, err.message);
    return "HOLD";
  }
}

module.exports = { decide25TemaFIx, calculateTEMA, calculateATR };
