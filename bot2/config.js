...
console.log(
  `   Trading Pairs: ${config.symbols.length} (${config.symbols.join(", ")})`
);
console.log(
  `   Primary Timeframe: ${config.timeframe} | Secondary: ${config.timeframe15m}`
);
console.log(`   Leverage: ${config.leverage}x`);
console.log(`   Position Size: $${config.positionSizeUSDT}`);
console.log(
  `   TEMA Periods (5m): ${config.tema.fast}/${config.tema.medium}/${config.tema.slow}`
);
console.log(
  `   TEMA 100 (15m): ${config.tema.tema100} - Secondary trend filter`
);
console.log(`   Min Signal Score: ${config.minSignalScore}/8 (with DMI)`);
console.log(
  `   Candles Required: ${config.minCandlesRequired} (fetching ${config.candleFetchLimit})`
);
